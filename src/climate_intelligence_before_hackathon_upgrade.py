...
class ClimateLendingIntelligence:
    ...
    def build_historical_feature_table(self) -> pd.DataFrame:
        ...
        hist["flood_risk_index"] = normalize_series(hist["flood_events"])
        hist["cyclone_risk_index"] = 0.6 * normalize_series(hist["cyclone_events"]) + 0.4 * hist["wind_severity"]
    ...
    def explain_decision(tenure_risk_score: float, flood_risk: float, cyclone_risk: float, heat_risk: float) -> Tuple[str, str]:
        if tenure_risk_score > 0.70:
            decision = "Not Approved"
        elif tenure_risk_score > 0.50:
